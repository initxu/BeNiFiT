...
def main():
    ...
    for identifier in tqdm(identifier_ls):
        ...
        prompt=nft_prompt.replace("$[Historical Start]$", historical_start_date[nft_historical_period]).replace("$[Historical End]$", historical_end).replace("$[Historical Trading Data]$", nft_historical_data).replace("$[Rarity information]$", nft_rarity_rank_info).replace("$[Collection Description]$", collection_description).replace("$[Collection Trading Infomation]$", collection_trading_info)
